...
export const authApi = {
  async login(credentials: LoginForm): Promise<ApiResponse<{ user: User; token: string; refreshToken: string }>> {
    try {
      ...
      return response.data as ApiResponse<{ user: User; token: string; refreshToken: string }>;
    } catch (error: unknown) {
      return {
        success: false,
        message: (error as any).response?.data?.message || 'Login failed',
        error: (error as any).response?.status === 401 ? 'Invalid credentials' : 'Login failed'
      };
    }
  },
  ...
  async register(userData: RegisterForm): Promise<ApiResponse<{ user: User; token: string; refreshToken: string }>> {
    try {
      ...
      return {
        ...
        error: (error as any).response?.status === 409 ? 'User already exists' : 'Registration failed'
      };
    }
  },
  ...
  async logout(): Promise<ApiResponse<void>> {
    try {
      ...
      return {
        success: false,
        message: (error as any).response?.data?.message || 'Logout failed',
        error: 'Logout failed'
      };
    }
  },
  ...
  async refreshToken(refreshToken: string): Promise<ApiResponse<{ token: string; refreshToken: string }>> {
    try {
      ...
      return response.data as ApiResponse<{ token: string; refreshToken: string }>;
    } catch (error: unknown) {
      return {
        success: false,
        message: (error as any).response?.data?.message || 'Token refresh failed',
...
export const connectionApi = {
  async sendConnectionRequest(userId: string, message?: string): Promise<ApiResponse<void>> {
    try {
      const response = await apiClient.post(`/api/connections/connect/${userId}`, { message });
      return response.data as any;
    } catch (error: any) {
      return {
        success: false,
  ...
  async acceptConnectionRequest(connectionId: string): Promise<ApiResponse<void>> {
    try {
      const response = await apiClient.post(`/api/connections/respond/${connectionId}`, { action: 'accept' });
      return response.data as any;
    } catch (error: any) {
      return {
        success: false,
  ...
  async declineConnectionRequest(connectionId: string): Promise<ApiResponse<void>> {
    try {
      const response = await apiClient.post(`/api/connections/respond/${connectionId}`, { action: 'decline' });
      return response.data as any;
    } catch (error: any) {
      return {
        success: false,
        message: error.response?.data?.message || 'Failed to decline connection request',
        error: 'Failed to decline connection request'
      };
    }
  },

  async getConnections(page = 1, limit = 20): Promise<ApiResponse<{ connections: Connection[], pagination: any }>> {
    try {
      const response = await apiClient.get(`/api/connections?page=${page}&limit=${limit}&status=accepted`);
      return response.data as any;
    } catch (error: any) {
      return {
        success: false,
  ...
  async getConnectionRequests(type: 'received' | 'sent'): Promise<ApiResponse<{ requests: Connection[], pagination: any }>> {
    try {
      const endpoint = type === 'received' ? '/api/connections/requests' : '/api/connections/sent';
      const response = await apiClient.get(endpoint);
      return response.data as any;
    } catch (error: any) {
      return {
  ...
  async getSuggestions(): Promise<ApiResponse<{ suggestions: User[] }>> {
    try {
      ...
      return {
        success: false,
        message: error.response?.data?.message || 'Failed to fetch connection suggestions',
